...
(async()=>{
  ...
  await page.waitForFunction(()=>document.querySelector('#kuMethodChoice')?.textContent.includes('XGBoost Regression'));
  let text=await page.locator('#kuMethodChoice').textContent();
  ...
  await page.selectOption('#analysisTarget','Income');
  await page.waitForFunction(()=>document.querySelector('#kuMethodChoice')?.textContent.includes('Spearman Correlation'));
  text=await page.locator('#kuMethodChoice').textContent();
