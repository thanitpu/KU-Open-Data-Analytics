...
(async()=>{
  ...
  w.document.getElementById('continuePrepare').click();await tick();assert.ok(w.document.getElementById('journeyPendingView').textContent.includes('Preparation summary'));let group=w.document.getElementById('prepareGroupField');group.value='Group';group.dispatchEvent(new w.Event('change',{bubbles:true}));await tick();assert.strictEqual(w.document.getElementById('continueSetup').disabled,false);w.document.getElementById('continueSetup').click();await tick();await tick();assert.ok(w.document.getElementById('journeyPendingView').textContent.includes('Recommended Setup'));assert.strictEqual(w.document.getElementById('runAnalysisBtn').disabled,false);w.document.getElementById('runAnalysisBtn').click();await tick();await tick();assert.ok(w.document.querySelector('.result-answer').textContent.includes('One-way ANOVA'));state=w.KUAppState.getState();assert.strictEqual(state.currentStep,'results');

  // Explain-drivers results surface model-derived importance, not only performance metrics.
  w.goToJourneyStep('analyze');await tick();w.document.querySelector('[data-question-type="explain-drivers"]').click();target=w.document.getElementById('analysisTarget');target.value='Score';target.dispatchEvent(new w.Event('change',{bubbles:true}));w.KUAppState.setResultPayload({result:{status:'COMPLETE',route:'regression',analysis_type:'regression',target:'Score',method:{model:'XGBoost'},evidence:{r2:.81,rmse:2.1,mae:1.4},findings:[{relationship:'Age',importance:.42,effect:.42},{relationship:'Group_B',importance:.28,effect:.28},{relationship:'Satisfaction_High',importance:.17,effect:.17}],warnings:['Feature importance is predictive model evidence and does not establish causal drivers.']},report:{overview:[],method:[],evidence:[],findings:[]}});w.goToJourneyStep('results');await tick();assert.ok(w.document.querySelector('.result-answer h2').textContent.includes('strongest predictive signals'));assert.ok(w.document.getElementById('predictiveDrivers'));assert.strictEqual(w.document.querySelector('.result-stale'),null,'fresh result should not be marked stale');

  // Predictor edits preserve the result but disclose that it belongs to an earlier plan.
  ...
  // Measurement-level edits re-derive the route without silently deleting the previous result.
