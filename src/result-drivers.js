...
function renderDriverAnswer(){
  ...
  const r=state.result?.payload?.result,findings=Array.isArray(r?.findings)?r.findings.filter(x=>Number.isFinite(Number(x?.importance??x?.effect))):[];
  if(!findings.length)return;
  const top=findings.slice(0,5),target=state.analysisPlan.target||r.target||'the outcome',names=top.slice(0,3).map(x=>x.relationship).filter(Boolean);
  const headline=document.querySelector('.result-answer h2');
  if(headline)headline.textContent=`The strongest predictive signals for ${target} were ${names.join(', ')}.`;
  ...
  section.innerHTML=`<div class="head">Predictive drivers</div><div class="body"><div class="table"><table><thead><tr><th>Feature</th><th>Model importance</th></tr></thead><tbody>${top.map(x=>`<tr><td>${safe(x.relationship||'Feature')}</td><td>${Number(x.importance??x.effect).toFixed(4)}</td></tr>`).join('')}</tbody></table></div><div class="note" style="margin-top:10px">These are model-derived predictive importance signals from the fitted validated model. They do not establish causal effects.</div></div>`;
