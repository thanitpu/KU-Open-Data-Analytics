// KU Open DA — local Profile Manifest v1. Aggregated statistics only; no raw rows leave the browser.
(function(root,factory){
  const api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  if(root)root.KUProfileManifest=api;
...
function categoricalProfile(values,name,maxCategories){const vals=values.map(v=>String(v)),counts=new Map();for(const v of vals)counts.set(v,(counts.get(v)||0)+1);const entries=[...counts.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])),n=vals.length,rareLimit=Math.max(2,Math.floor(n*.01)),rare=entries.filter(([,c])=>c<=rareLimit),redacted=sensitiveLike(name)||structuralLike(name,n,entries.length);return{redacted,redacted_reason:redacted?'identifier_or_sensitive_like_field':null,top:redacted?[]:entries.slice(0,maxCategories).map(([value,count])=>({value,count,pct:100*count/n})),other_count:redacted?null:entries.slice(maxCategories).reduce((s,[,c])=>s+c,0),dominant_pct:entries.length?100*entries[0][1]/n:0,rare_level_count:rare.length,rare_observation_pct:100*rare.reduce((s,[,c])=>s+c,0)/Math.max(1,n),entropy_normalized:entropyNormalized(counts)}}
function temporalNameHint(name){return/(^|_)(date|dt|time|timestamp)(_|$)/i.test(String(name||''))||/(date|timestamp)$/i.test(String(name||''))}
function temporalProfile(values,name,storage){const vals=values.filter(v=>!missing(v));if(!vals.length)return null;if(storage==='numeric'&&!temporalNameHint(name))return null;const isoLike=vals.filter(v=>typeof v==='string'&&/^\d{4}[-/]\d{1,2}[-/]\d{1,2}/.test(v.trim())).length;const hinted=temporalNameHint(name)||isoLike/vals.length>=.5;if(!hinted)return null;const parsed=vals.map(v=>Date.parse(v)).filter(Number.isFinite);if(parsed.length/vals.length<.8)return null;const uniq=[...new Set(parsed)].sort((a,b)=>a-b),diffs=uniq.slice(1).map((v,i)=>(v-uniq[i])/86400000).filter(v=>v>0),med=diffs.length?quantile(diffs,.5):null,m=diffs.length?mean(diffs):null,sd=diffs.length>1?sampleSD(diffs):null,cv=m&&Number.isFinite(sd)?sd/m:null,mm=minMax(parsed);let granularity='single_point';if(med!==null){if(med<1)granularity='subdaily';else if(med<=2)granularity='daily';else if(med<=8)granularity='weekly';else if(med<=40)granularity='monthly';else if(med<=100)granularity='quarterly';else if(med<=400)granularity='yearly';else granularity='irregular_long_interval'}return{detected:true,parse_rate:parsed.length/vals.length,min:new Date(mm.min).toISOString(),max:new Date(mm.max).toISOString(),unique_timestamps:uniq.length,median_interval_days:med,interval_cv:cv,granularity}}
function deterministicSample(data,limit=DEFAULT_PROFILE_ROW_LIMIT){const n=data.length,k=Math.max(1,Math.min(n,Number(limit)||DEFAULT_PROFILE_ROW_LIMIT));if(n<=k)return{rows:data,mode:'full',method:'full_dataset',fraction:1};const rows=new Array(k),step=n/k;for(let i=0;i<k;i++)rows[i]=data[Math.min(n-1,Math.floor((i+.5)*step))];return{rows,mode:'sampled',method:'deterministic_systematic',fraction:k/n}}
function duplicateRows(headers,data){const seen=new Set();let dup=0;for(const row of data){const key=headers.map(h=>String(row[h]??'')).join('\u001f');if(seen.has(key))dup++;else seen.add(key)}return dup}
function build({headers=[],data=[],types={},meta={},analysisIntent={},maxCategories=20,histogramBins=12,profileRowLimit=DEFAULT_PROFILE_ROW_LIMIT}={}){
  if(!Array.isArray(headers)||!Array.isArray(data))throw new Error('headers and data must be arrays');
  const datasetRows=data.length,sample=deterministicSample(data,profileRowLimit),profileData=sample.rows,fields=[];let sampleMissingCells=0;
  for(const name of headers){const observed=profileData.map(r=>r?.[name]).filter(v=>!missing(v)),miss=profileData.length-observed.length,unique=new Set(observed.map(v=>String(v))).size,storage=types?.[name]||((observed.every(v=>finite(v)!==null)&&observed.length)?'numeric':'text'),level=meta?.[name]?.level||null;sampleMissingCells+=miss;const numeric=storage==='numeric'?numericProfile(observed,histogramBins):null,temporal=temporalProfile(observed,name,storage),role=analysisIntent?.target===name?'target':'predictor';const field={name,role,storage_type:storage,measurement_level:level,profile:{n:observed.length,missing:miss,missing_pct:profileData.length?100*miss/profileData.length:0,unique,unique_ratio:observed.length?unique/observed.length:0,basis:sample.mode}};if(numeric){Object.assign(field.profile,numeric.summary);field.distribution=numeric.distribution;field.outliers=numeric.outliers}else field.frequency=categoricalProfile(observed,name,maxCategories);if(temporal)field.temporal=temporal;field.privacy={frequency_values_redacted:Boolean(field.frequency?.redacted)};fields.push(field)}
