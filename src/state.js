...
(function(root){
  ...
  function setDataset(dataset){const next=dataset&&dataset.loaded!==false?{loaded:true,name:dataset.name||null,rowCount:Number(dataset.rowCount)||0,columnCount:Number(dataset.columnCount)||0,fields:Array.isArray(dataset.fields)?copy(dataset.fields):[]}:{loaded:false,name:null,rowCount:0,columnCount:0,fields:[]};const datasetChanged=state.dataset.loaded&&datasetIdentity(state.dataset)!==datasetIdentity(next);if(datasetChanged){const fresh=initialState();state={...fresh,dataset:next,currentStep:'start'};emit('dataset:replace+analysis-reset');return}state={...state,dataset:next};emit('dataset:update')}
  function updateAnalysisPlan(patch={}){const before=state.analysisPlan;const next={...before,...patch};if(Object.prototype.hasOwnProperty.call(patch,'predictors'))next.predictors=unique(patch.predictors);if(patch.preparation)next.preparation={...before.preparation,...patch.preparation};if(patch.setup)next.setup={...before.setup,...patch.setup};const questionOrTargetChanged=next.questionType!==before.questionType||next.target!==before.target;const predictorsChanged=JSON.stringify(next.predictors)!==JSON.stringify(before.predictors)||next.predictorMode!==before.predictorMode;const routeChanged=next.route!==before.route||next.analyticalFamily!==before.analyticalFamily;if(questionOrTargetChanged){next.preparation=emptyPreparation();next.setup=emptySetup()}else if(predictorsChanged||routeChanged){next.preparation={status:'needs-review',approved:false};next.setup=emptySetup()}state={...state,analysisPlan:next,result:questionOrTargetChanged?emptyResult():state.result};emit(questionOrTargetChanged?'analysis-plan:update+downstream-reset+result-reset':predictorsChanged?'analysis-plan:predictors+downstream-reset':routeChanged?'analysis-plan:route+downstream-reset':'analysis-plan:update')}
  ...
  function setResultPayload(payload,{validated=true,source='api'}={}){const p=state.analysisPlan;const planSnapshot={questionType:p.questionType,target:p.target,predictors:copy(p.predictors),predictorMode:p.predictorMode,analyticalFamily:p.analyticalFamily,route:p.route,question:p.question,preparation:copy(p.preparation)};state={...state,result:{payload:copy(payload),validated:Boolean(validated),source,lastRunAt:new Date().toISOString(),planSnapshot}};emit('result:set')}
