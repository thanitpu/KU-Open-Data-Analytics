...
(function(root){
  ...
  const emptyDataset=()=>({loaded:false,name:null,rowCount:0,columnCount:0,revision:0,fields:[],origin:'local',assets:[],approval:null,acquiredAt:[],effectiveAt:[]});
  const emptyTextAnalysis=()=>({selectedTextField:null,labelField:null,profile:null,terms:[],phrases:[],sentiment:null,topics:null,topicSentiment:null,semantic:null,curation:{},derivedFeatures:[]});
  const emptyMethodSelection=()=>({methodMode:'recommended',selectedMethods:[]});
  const initialState=()=>({currentStep:'start',dataset:emptyDataset(),analysisPlan:{questionType:null,target:null,predictors:[],predictorMode:'all-suitable',analyticalFamily:null,route:null,question:'',...emptyMethodSelection(),preparation:emptyPreparation(),setup:emptySetup()},result:emptyResult(),textAnalysis:emptyTextAnalysis()});
  ...
  function datasetIdentity(d){return `${d.loaded}|${d.revision||0}|${d.name||''}|${d.origin||'local'}|${d.rowCount}|${d.columnCount}|${(d.fields||[]).map(f=>f.name||'').join('\u001f')}|${(d.assets||[]).map(a=>a.dataAssetId||'').join('\u001f')}`}
  ...
  function setDataset(dataset){
    const next=dataset&&dataset.loaded!==false?{loaded:true,name:dataset.name||null,rowCount:Number(dataset.rowCount)||0,columnCount:Number(dataset.columnCount)||0,revision:Number(dataset.revision)||0,fields:Array.isArray(dataset.fields)?copy(dataset.fields):[],origin:dataset.origin||'local',assets:Array.isArray(dataset.assets)?copy(dataset.assets):[],approval:dataset.approval?copy(dataset.approval):null,acquiredAt:Array.isArray(dataset.acquiredAt)?copy(dataset.acquiredAt):[],effectiveAt:Array.isArray(dataset.effectiveAt)?copy(dataset.effectiveAt):[]}:emptyDataset();
  ...
  function updateTextAnalysis(patch={}){state={...state,textAnalysis:{...state.textAnalysis,...copy(patch),curation:Object.prototype.hasOwnProperty.call(patch,'curation')?copy(patch.curation||{}):state.textAnalysis.curation}};emit('text-analysis:update')}
  function updateTextCuration(topicId,patch={}){const key=String(topicId),current=state.textAnalysis.curation?.[key]||{};state={...state,textAnalysis:{...state.textAnalysis,curation:{...state.textAnalysis.curation,[key]:{...current,...copy(patch)}}}};emit('text-analysis:curation')}
  function resetTextAnalysis(){state={...state,textAnalysis:emptyTextAnalysis()};emit('text-analysis:reset')}
  ...
  root.KUAppState=Object.freeze({getState,subscribe,setStep,setDataset,updateAnalysisPlan,setPredictors,setPreparation,setSetup,setResultPayload,resetResult,resetAnalysis,updateTextAnalysis,updateTextCuration,resetTextAnalysis,canEnterStep});
