(function(root){
  ...
  function candidates(){return (state().dataset?.fields||[]).map(field=>root.KUTextFieldDetector.detect({fieldName:field.name,values:values(field.name)})).filter(item=>item.semanticType!=='empty').sort((a,b)=>(a.semanticType==='text'?0:1)-(b.semanticType==='text'?0:1)||b.confidence-a.confidence);}
  ...
  function renderProfile(){
    ...
    const suggested=found.find(item=>item.semanticType==='text');
    if(!current.selectedTextField&&suggested)setTimeout(()=>chooseTextField(suggested.fieldName),0);
    const profile=current.profile;
    node.innerHTML=`<section class="card text-card"><div class="head">Text field profile <span class="text-badge">deterministic</span></div><div class="body"><label class="text-control"><span>Text field</span><select onchange="KUTextAnalyticsWorkflow.selectField(event)"><option value="">Choose a field</option>${found.map(item=>`<option value="${esc(item.fieldName)}" ${item.fieldName===current.selectedTextField?'selected':''}>${esc(item.fieldName)} · ${item.semanticType==='text'?'suggested text':esc(item.semanticType)} · ${Math.round(item.confidence*100)}%</option>`).join('')}</select></label><p class="note">Suggested text fields appear first. You may explicitly choose another non-empty field; detection is guidance, not authority.</p>${profile?`<div class="text-metrics"><span><b>${profile.documents.observed}</b> observed</span><span><b>${fmt(profile.documents.duplicatePct)}%</b> duplicates</span><span><b>${fmt(profile.length.median)}</b> median chars</span><span><b>${profile.language.counts.th||0}</b> Thai</span><span><b>${profile.language.counts.en||0}</b> English</span><span><b>${profile.language.counts.mixed||0}</b> mixed</span></div><p class="note">Profiled ${profile.provenance.profileRows} of ${profile.provenance.datasetRows} rows${profile.provenance.sampled?' using deterministic sampling':''}. No rows were modified.</p>`:'<div class="empty">Choose a field to profile as text.</div>'}</div></section>${profile?`<div class="text-grid"><section class="card"><div class="head">Terms</div><div class="body text-chip-list">${current.terms.map(item=>`<span>${esc(item.term)} <b>${item.count}</b></span>`).join('')||'<div class="empty">No repeated terms.</div>'}</div></section><section class="card"><div class="head">Phrases</div><div class="body text-chip-list">${current.phrases.map(item=>`<span>${esc(item.phrase)} <b>${item.count}</b></span>`).join('')||'<div class="empty">No repeated phrases.</div>'}</div></section></div>`:''}`;
  }
