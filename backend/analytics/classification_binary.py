...
def run_fast_binary_parity(
    ...
    for fold, (tr, va) in enumerate(outer_cv.split(X, y), 1):
        ...
        for itr, iva in inner_cv.split(Xtr, ytr):
            ...
            A = prep_inner.fit_transform(Xtr.iloc[itr])
            B = prep_inner.transform(Xtr.iloc[iva])
        ...
        A = prep_outer.fit_transform(Xtr)
        B = prep_outer.transform(Xva)
    ...
    for tr, va in full_cv.split(X, y):
        prep = _binary_parity_preprocessor(X.iloc[tr])
    ...
    final_preprocessor = _binary_parity_preprocessor(X)
    Xfull = final_preprocessor.fit_transform(X)
    ...
    result = {
        ...
        "findings": findings,
        "warnings": warnings,
        "readiness": "FAST_PARITY_VALIDATED",
