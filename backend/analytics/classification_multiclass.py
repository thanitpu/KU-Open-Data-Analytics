...
def _build_fast_multiclass_features(X):
    ...
    for a, b in pairs:
        if a in Z.columns and b in Z.columns:
            Z[f"{a}__x__{b}"] = (
                pd.to_numeric(Z[a], errors="coerce")
                * pd.to_numeric(Z[b], errors="coerce")
            )
    return Z


def run_fast_multiclass_classification(
    df,
    target,
    ...
    if len(classes) < 3:
        raise ValueError(
            f"Multiclass classification requires ≥3 classes; found {len(classes)}."
        )
    ...
    for tr, va in cv.split(X, y):
        prep = _classification_preprocessor(X.iloc[tr])
        Xtr = prep.fit_transform(X.iloc[tr])
        Xva = prep.transform(X.iloc[va])
        model = clone(base_model)
        sw = compute_sample_weight(class_weight="balanced", y=y.iloc[tr])
        model.fit(Xtr, y.iloc[tr], sample_weight=sw)
        prob = np.asarray(model.predict_proba(Xva), dtype=np.float64)
        prob = np.clip(prob, 1e-12, None)
        prob /= prob.sum(axis=1, keepdims=True)
        oof_prob[va] = prob

    pred = np.argmax(oof_prob, axis=1)
    top_sorted = np.sort(oof_prob, axis=1)
    confidence = top_sorted[:, -1]
    ...
    evidence = {
        ...
        "roc_auc_ovr_macro": float(
            roc_auc_score(y, oof_prob, multi_class="ovr", average="macro")
        ),
    ...
    if accepted.sum() > 0:
        evidence["selective_accuracy"] = float(
            np.mean(pred[accepted] == y.to_numpy()[accepted])
        )
        evidence["selective_macro_f1"] = float(
            f1_score(
                y.to_numpy()[accepted],
                pred[accepted],
                average="macro",
        ...
        evidence["selective_accuracy"] = None
        evidence["selective_macro_f1"] = None

    prep_full = _classification_preprocessor(X)
    Xfull = prep_full.fit_transform(X)
    model_full = clone(base_model)
    sw_full = compute_sample_weight(class_weight="balanced", y=y)
    model_full.fit(Xfull, y, sample_weight=sw_full)
    ...
    artifacts = {
        ...
        "inverse_class_mapping": int_to_class,
