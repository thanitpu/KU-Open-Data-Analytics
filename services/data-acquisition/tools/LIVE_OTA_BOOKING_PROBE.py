...
def main():
    ...
    payload = {
        ...
        'guardrail': 'This probe cannot approve or store production data. It tests public search results only with explicit dates, occupancy and currency.',
    }
    out = ROOT / args.output
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')
    print(json.dumps({
        'first': first.get('diagnostics'), 'second': second.get('diagnostics'),
