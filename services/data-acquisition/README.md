...
`config/coffee_recovery_next_step.json` and `docs/COFFEE_RECOVERY_NEXT_STEP.md` compare exactly three post-run choices without making a request: one same-URL Coffee rerun with the hardened detector, moving to Q-Diving recovery, or stopping Coffee for now. The decision keeps detector false-positive risk, environment/access risk, unevaluated live extraction, and repaired diagnostic retention distinct; it does not reinterpret the completed exit-2 observation as success.

The deterministic score recommends the separately queued Coffee rerun because it is the smallest action that can directly resolve Roots/Nana product, price, provenance, and repeatability gaps. Its staged four-attempt maximum stops a source after an unusable first observation, permits a repeat only after a strict first product record, and preserves evidence-before-exit, cloud-default, no-escalation, candidate-only, production-disabled, and scheduler-null boundaries. The recommendation is not execution authority.

## Acquisition-to-Analysis Handoff v1

`knowledge/v1/acquisition-analysis-handoff.schema.json`, policy `KU2D-KP-000003`, and `acquisition/acquisition_analysis_handoff.py` define the durable boundary between source acquisition and KU2A. Acquisition accepts every technically valid, authorized, policy-compliant, provenance-bearing and sanitized record for Analysis without making semantic relevance, quality, rank, analytical deduplication, final inclusion or production claims.

The first manifest, `KU2D-AI-000001`, indexes all ten immutable P50 Q-Diving YouTube candidates with packet, Result, artifact, commit, hash and per-record provenance. Analysis owns all later semantic decisions. Historical Human Review and `selection_target` fields remain readable but are not active Acquisition completion gates. This alignment performs no provider request or quota use and grants no production write, approval or scheduling authority.
