...
Explicit baht displays normalize without hidden scaling. Each amount is now retained as a surface-scoped price observation with its raw display, explicit currency, price role, visible cue, source surface, timestamp, product identity, explicit-only variant identity, shop/seller identity, conditional state, and sanitized rendered-DOM provenance. Roles distinguish current, original, variation minimum/maximum, from-price, promotional, voucher/conditional, member/account-conditional, and unknown display prices. The compatibility `current_price` scalar represents only an explicitly evidenced current or promotional selling price. From-price, variation, voucher, member, and unlabeled display amounts remain in `price_observations` and are never silently promoted to `current_price`.
